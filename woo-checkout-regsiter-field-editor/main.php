...
add_action( 'plugins_loaded', 'jwcfe_load_textdomain' );

function jwcfe_load_textdomain() {
...
add_action( 'admin_enqueue_scripts', 'jwcfe_admin_scripts', 10, 1 );
    ...
    if (get_option('jwcfe_account_sync_fields') && get_option('jwcfe_account_sync_fields') == "on") {
        $fields = array_merge(
            JWCFE_WC_Checkout_Field_Editor::get_fields('account'),
            JWCFE_WC_Checkout_Field_Editor::get_fields('billing'),
            JWCFE_WC_Checkout_Field_Editor::get_fields('shipping'),
		...
		if (is_array($fields) && !empty($fields)) {
			...
			foreach ($fields as $name => $options) {
				...
				if (isset($options['show_in_order']) && $options['show_in_order'] && $enabled && $is_custom_field) {
					...
					if (!empty($value)) {
						$label = isset($options['label']) && !empty($options['label']) ? __($options['label'], 'jwcfe') : $name;
						$formatted_value = wptexturize($value);
	
						if (is_account_page()) {
							if (apply_filters('jwcfe_view_order_customer_details_table_view', true)) {
								$fields_html .= '<tr><th>' . esc_attr($label) . ':</th><td>' . $formatted_value . '</td></tr>';
							} else {
								$fields_html .= '<br/><dt>' . esc_attr($label) . ':</dt><dd>' . $formatted_value . '</dd>';
							}
						} else {
